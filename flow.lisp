...
(defmethod extend-flow-state ((state flow-state))
  (when (get-next-stage (flow state) (stage state))
    (make-instance 'next-flow-state :name (name state) :stage (get-next-stage (flow state) (stage state)) :flow (flow state))))
...
(defgeneric get-method (state)
  (:documentation "Returns the method that should be called for the given state"))
(defmethod get-method (state)
  (or (flow-method (flow state) (stage state) (name state))
      (let ((extended-flow-state (extend-flow-state state)))
	(when extended-flow-state 
	  (get-method extended-flow-state)))))
